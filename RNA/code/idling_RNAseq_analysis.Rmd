...
```{r Data assembly, eval=TRUE, include=TRUE, echo=TRUE}

hosts <- c("https://useast.ensembl.org",
...
# Try the different mirror sites, break out of the loop when one works
for(host in hosts){
  message(paste('Trying', host))
  try({ 
    ensembl <- f(host) 
    break 
  }, silent = FALSE)
...
fn <- file.path("..", "data", "SKMEL5_subclone_BRAFi_timecourse_RNAseq_data.csv")
...
```{r}
d <- read.csv(
  file.path("..", "data", "SKMEL5_subclone_BRAFi_timecourse_RNAseq_data.csv"), 
  row.names=1)
```
...
# Try the different mirror sites (defined above), break out of the loop when one works
for(host in hosts){
  message(paste('Trying', host))
  try({ 
    ensembl <- f(host) 
...
```{r fig.height=10, fig.width=5, eval=FALSE}
if(SAVEPLOT) 
    pdf(file="ca-genes_fold-change_heatmap.pdf", height=10, width=5)
...
if(SAVEPLOT) 
    pdf(file="ca-genes_fold-change_heatmap.pdf", height=10, width=5)
hm2 <- heatmap.2(as.matrix(b[,!grepl("_0",colnames(b))]),
...
library(circlize)
pdf(file="ca-genes_fold-change_ComplexHeatmap.pdf", height=14)

x <- as.matrix(b[,!grepl("_0",colnames(b))])
ceil_x <- ceiling(max(abs(x)))

col_fun = colorRamp2(seq(-ceil_x, ceil_x, ceil_x / 10), bluered(21))

panel_fun = function(index, nm) {
    grid.rect(height = unit(1.5, "npc"), just = c('center', 'bottom'),
              gp = gpar(lwd = 0))
    grid.text(nm, 0.5, 0.75, just = c('center', 'bottom'), 
              gp = gpar(fontsize = 20, fontface = 'bold'))
}

column_ha <- HeatmapAnnotation(
  subclones = anno_block(align_to = list("SC01" = 1:2, "SC07" = 3:4, "SC10" = 5:6)),
  days = anno_text(c("3d", "8d", "3d", "8d", "3d", "8d"), rot = 0, 
                   just = 'center', location = unit(0.6, "npc"), 
                   gp = gpar(fontsize = 20))
)

hm <- Heatmap(x, name="Log2 Fold Change", col = col_fun, row_names_gp = gpar(fontsize = 14), 
        show_column_names = FALSE, cluster_columns = FALSE, cluster_rows = FALSE, 
        heatmap_height = unit(12.5, "in"), top_annotation = column_ha,
...
if(!file.exists(fn) | OVERWRITE) 
  write.csv(z, file.path("..", "data", "SKMEL5_subclone_BRAFi_l2FC_d3and8.csv"))
```
