...
d <- addMapInfo(dat_melt, file.path("..", "platemaps", "platemap_20210211.tsv"))
```
...
avgd <- d %>%
  ...
  dplyr::summarise(mean = mean(Intensity), se = sd(Intensity)/sqrt(8), n = n())
...
ggplot() +
  ...
  geom_ribbon(aes(x=Time, ymin=selow, ymax=sehigh, group=drug1, fill=drug1), 
              alpha=0.2) +
  theme(legend.position.inside = c(0.2,0.8), legend.text = element_text(size = 12),
...
ggplot() +
  geom_rect(data = data.frame(x = c(0)), 
            aes(xmin = 280, xmax = Inf, ymin = -Inf, ymax = 1.15),
                   fill = "orange", color="black", size=0, alpha = 0.05) +
  geom_rect(data = data.frame(x = c(0)), 
            aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1.15),
...
ggsave("SOCE_untreated_idling.pdf", width = 8, height = 5)
#################################
```
