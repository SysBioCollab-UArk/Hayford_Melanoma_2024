...
```{r}
SAVEPLOTS <- FALSE  # whether to save plots as PDF files
```
...
```{r Generate plots, fig.height=6, fig.width=3, warning=FALSE}
ggplot(RSL3_UTvI_all, aes(x=conc, y=p, color=State, fill=State)) +
  facet_wrap(~drug2, ncol=1) +
  ...
  geom_text(data = IC50_df[IC50_df$State=="Idling",], aes(x = IC50, y=-0.04, label = paste(signif(IC50,2),"µM"), color = State, text.size=6), nudge_x=-.3) +
  geom_text(data = IC50_df[IC50_df$State!="Idling",], aes(x = IC50, y=-0.04, label = paste(signif(IC50,2),"µM"), color = State, text.size=6), nudge_x=.3) +
...
if(SAVEPLOTS)   ggsave("RSL3-all_DRC_2024.pdf", width = 3, height = 6)
```
